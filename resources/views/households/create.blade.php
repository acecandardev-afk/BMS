...
<div class="py-4" style="max-width: 56rem;">
    <div class="d-flex align-items-center gap-3 mb-4">
        ...
        <div>
            <h2 class="h5 mb-0 bc-page-title">Create Household</h2>
            <p class="small mb-0 bc-page-subtitle">Add a new household profile.</p>
        </div>
    </div>

    <form method="POST" action="{{ route('households.store') }}" class="card bc-card p-3 p-md-4 bc-form-panel">
        @csrf
        <div class="row g-3">
            <div class="col-12 col-md-6">
                <label class="form-label small fw-medium">Household Number</label>
                <input type="text" name="household_number" value="{{ old('household_number') }}" class="form-control @error('household_number') is-invalid @enderror" required>
                @error('household_number')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>
            <div class="col-12 col-md-6">
                <label class="form-label small fw-medium">Zone</label>
                <input type="text" name="zone" value="{{ old('zone') }}" class="form-control @error('zone') is-invalid @enderror" required>
            ...
            <div class="col-12">
                ...
                <select name="head_resident_id" class="form-select @error('head_resident_id') is-invalid @enderror">
                    ...
                    @foreach($residents as $resident)
                        <option value="{{ $resident->id }}" @selected(old('head_resident_id') == $resident->id)>
        ...
        <div class="d-flex gap-2 mt-4">
            <button type="submit" class="btn btn-bc-primary rounded-pill">Save Household</button>
            <a href="{{ route('households.index') }}" class="btn btn-outline-secondary rounded-pill">Cancel</a>
        </div>
    </form>
</div>
@endsection
