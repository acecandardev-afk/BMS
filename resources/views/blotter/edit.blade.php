...
@section('title', 'Edit Blotter Record')
...
<div class="py-4" style="max-width: 56rem;">
    ...
    <div class="d-flex align-items-center gap-3 mb-4">
        ...
        <div>
            <h2 class="h5 mb-0 bc-page-title">Edit Blotter Record</h2>
            <p class="small text-muted mb-0">{{ $blotter->blotter_number }}</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('blotter.update', $blotter) }}"
            ...
            <div class="row g-3">
                    ...
                    <select name="complainant_id" class="form-select form-select-sm">
                        ...
                        @foreach($residents as $resident)
                            <option value="{{ $resident->id }}"
                                {{ old('complainant_id', $blotter->complainant_id) == $resident->id ? 'selected' : '' }}>
                ...
                <div class="col-12 col-sm-6">
                    ...
                    <select name="respondent_id" class="form-select form-select-sm">
                        ...
                        @foreach($residents as $resident)
                            <option value="{{ $resident->id }}"
                                {{ old('respondent_id', $blotter->respondent_id) == $resident->id ? 'selected' : '' }}>
                                {{ $resident->full_name }} — {{ $resident->zone }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="row g-3">
                <div class="col-12 col-sm-6">
                    ...
                    <select name="incident_type" required
                        ...
                        @foreach($incident_types as $key => $label)
                            <option value="{{ $key }}"
                                {{ old('incident_type', $blotter->incident_type) === $key ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-12">
                    ...
                    <textarea name="narrative" rows="5" required
                              class="form-control form-control-sm {{ $errors->has('narrative') ? 'is-invalid' : '' }}">{{ old('narrative', $blotter->narrative) }}</textarea>
                    ...
                    <select name="assigned_to" class="form-select form-select-sm">
                        ...
                        @foreach(\App\Models\User::whereIn('role', ['admin', 'staff'])->get() as $officer)
                            <option value="{{ $officer->id }}"
                                {{ old('assigned_to', $blotter->assigned_to) == $officer->id ? 'selected' : '' }}>
                                {{ $officer->name }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="d-flex align-items-center gap-3">
            ...
            <a href="{{ route('blotter.show', $blotter) }}" class="btn btn-outline-secondary btn-sm">
                Cancel
            </a>
        </div>
