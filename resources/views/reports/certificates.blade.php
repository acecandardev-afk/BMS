...
<div class="py-4">
    ...
    <div class="d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between gap-3 mb-4">
        <div class="d-flex align-items-center gap-3 min-w-0">
            ...
            <div class="min-w-0">
                <h2 class="h5 mb-0 bc-page-title">Certificates Report</h2>
                <p class="small mb-0 bc-page-subtitle">Certificate request analytics for {{ $year }}.</p>
            </div>
        </div>
        ...
        <form method="GET" class="bc-filter-bar bc-filter-bar--embed" aria-label="Select report year">
            <div class="bc-filter-bar-inner">
                <div class="bc-filter-field-select">
                    <label for="rep-cert-year" class="visually-hidden">Year</label>
                    <select id="rep-cert-year" name="year" class="form-select form-select-sm" title="Year">
    ...
    <div class="row g-3 mb-4">
        <div class="col-12 col-sm-4">
            <div class="card bc-card p-3 p-md-4 bc-form-panel text-center">
                ...
                <p class="small text-muted mt-1 mb-0">Total Requests</p>
            </div>
        </div>
        <div class="col-12 col-sm-4">
            <div class="card bc-card p-3 p-md-4 bc-form-panel text-center">
                <p class="display-6 fw-bold text-success mb-0">
                    {{ number_format($data['by_status']['approved'] ?? 0) }}
                </p>
                <p class="small text-muted mt-1 mb-0">Approved</p>
            </div>
        </div>
        <div class="col-12 col-sm-4">
            <div class="card bc-card p-3 p-md-4 bc-form-panel text-center">
                <p class="display-6 fw-bold text-primary mb-0">
                    {{ $data['avg_turnaround'] ? $data['avg_turnaround'] . ' hrs' : 'N/A' }}
                </p>
                <p class="small text-muted mt-1 mb-0">Avg. Turnaround Time</p>
            </div>
        </div>
    </div>
    ...
    <div class="row g-4">
        ...
        <div class="col-12 col-lg-6">
            <div class="card bc-card p-3 p-md-4 bc-form-panel">
                <h3 class="small fw-semibold text-secondary mb-3">Requests by Certificate Type</h3>
                @php
                    $types = \App\Models\CertificateRequest::TYPES;
                    $maxType = count($data['by_type']) > 0 ? max($data['by_type']) : 1;
                    $typeColors = ['bg-primary', 'bg-success', 'bg-warning', 'bg-danger', 'bg-info'];
                @endphp
                @forelse($data['by_type'] as $type => $count)
                @php $colorIdx = array_search($type, array_keys($data['by_type'])); @endphp
                <div class="d-flex align-items-center gap-3 mb-3">
                    ...
                    <div class="bg-secondary bg-opacity-25 rounded-pill flex-shrink-0" style="width: 8rem; height: 0.4rem;">
                        <div class="{{ $typeColors[$colorIdx % count($typeColors)] }} rounded-pill"
        ...
        <div class="col-12 col-lg-6">
            <div class="card bc-card p-3 p-md-4 bc-form-panel">
                <h3 class="small fw-semibold text-secondary mb-3">Requests by Status</h3>
                @php
                    $statusColors = [
                        'pending'  => 'bg-warning',
                        'approved' => 'bg-success',
                ...
                <div class="d-flex align-items-center gap-3 mb-4">
                    ...
                    <div class="bg-secondary bg-opacity-25 rounded-pill flex-shrink-0" style="width: 8rem; height: 0.4rem;">
                        <div class="{{ $statusColors[$status] ?? 'bg-secondary' }} rounded-pill"
                             style="height: 0.4rem; width: {{ $totalStatus > 0 ? round(($count / $totalStatus) * 100) : 0 }}%;"></div>
                    </div>
                    <span class="small fw-bold text-dark flex-shrink-0" style="width: 2rem; text-align: right;">{{ $count }}</span>
        ...
        <div class="col-12">
            <div class="card bc-card p-3 p-md-4 bc-form-panel">
                <h3 class="small fw-semibold text-secondary mb-3">Monthly Request Volume ({{ $year }})</h3>
                @php
                ...
                <div class="d-flex align-items-end gap-2" style="height: 8rem;">
                    ...
                    <div class="flex-grow-1 d-flex flex-column align-items-center gap-1">
                        ...
                        <div class="w-100 rounded-top {{ $isCurrentMonth ? 'bg-primary' : 'bg-primary bg-opacity-25' }}"
